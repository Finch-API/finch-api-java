...
class SupportedBenefitTest {
    ...
    fun createSupportedBenefit() {
      ...
      assertThat(supportedBenefit.companyContribution().get()).containsExactly(SupportedBenefit.CompanyContribution.FIXED)
      assertThat(supportedBenefit.description()).contains("description")
      assertThat(supportedBenefit.employeeDeduction().get()).containsExactly(SupportedBenefit.EmployeeDeduction.FIXED)
      assertThat(supportedBenefit.frequencies().get()).containsExactly(BenefitFrequency.ONE_TIME)
      assertThat(supportedBenefit.hsaContributionLimit().get()).containsExactly(SupportedBenefit.HsaContributionLimit.INDIVIDUAL)
