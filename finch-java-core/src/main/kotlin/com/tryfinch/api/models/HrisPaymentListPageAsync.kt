...
class HrisPaymentListPageAsync private constructor(
    ...
    fun getNextPage(): CompletableFuture<Optional<HrisPaymentListPageAsync>> {
      return getNextPageParams().map {
        paymentsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<Payment>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<HrisPaymentListPageAsync>>.forEach(action: (Payment) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
