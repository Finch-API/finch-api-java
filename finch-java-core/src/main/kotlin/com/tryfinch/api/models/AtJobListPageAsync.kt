...
private constructor(
    ...
    fun getNextPageParams(): Optional<AtJobListParams> {
        if (!hasNextPage()) {
    ...
    fun getNextPage(): CompletableFuture<Optional<AtJobListPageAsync>> {
    ...
    constructor(
        ...
        fun forEach(action: Predicate<Job>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<AtJobListPageAsync>>.forEach(
                action: (Job) -> Boolean,
                executor: Executor
