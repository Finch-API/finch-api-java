...
class HrisIndividualRetrieveManyPageAsync private constructor(
    ...
    fun getNextPage(): CompletableFuture<Optional<HrisIndividualRetrieveManyPageAsync>> {
      return getNextPageParams().map {
        individualsService.retrieveMany(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class Response @JsonCreator constructor(
        ...
        fun _responses(): Optional<JsonField<List<IndividualResponse>>> = Optional.ofNullable(responses)
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<IndividualResponse>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<HrisIndividualRetrieveManyPageAsync>>.forEach(action: (IndividualResponse) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
