...
class HrisBenefitListSupportedBenefitsPageAsync private constructor(
    ...
    fun getNextPage(): CompletableFuture<Optional<HrisBenefitListSupportedBenefitsPageAsync>> {
      return getNextPageParams().map {
        benefitsService.listSupportedBenefits(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<SupportedBenefit>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<HrisBenefitListSupportedBenefitsPageAsync>>.forEach(action: (SupportedBenefit) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
