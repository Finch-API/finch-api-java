...
class HrisBenefitIndividualUnenrollManyPageAsync private constructor(
    ...
    fun getNextPage(): CompletableFuture<Optional<HrisBenefitIndividualUnenrollManyPageAsync>> {
      return getNextPageParams().map {
        individualsService.unenrollMany(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<UnenrolledIndividual>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<HrisBenefitIndividualUnenrollManyPageAsync>>.forEach(action: (UnenrolledIndividual) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
