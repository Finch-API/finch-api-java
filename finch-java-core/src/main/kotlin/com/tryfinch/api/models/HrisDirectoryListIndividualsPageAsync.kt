...
class HrisDirectoryListIndividualsPageAsync private constructor(
    ...
    fun hasNextPage(): Boolean {
      ...
      return paging().flatMap { it.offset()}.orElse(0) + individuals().count() < paging().flatMap { it.count()}.orElse(Long.MAX_VALUE);
    }
    ...
    fun getNextPageParams(): Optional<HrisDirectoryListIndividualsParams> {
      ...
      return Optional.of(HrisDirectoryListIndividualsParams.builder().from(params).offset(paging().flatMap { it.offset()}.orElse(0) + individuals().count()).build());
    }

    fun getNextPage(): CompletableFuture<Optional<HrisDirectoryListIndividualsPageAsync>> {
      return getNextPageParams().map {
        directoryService.listIndividuals(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class Response @JsonCreator constructor(
        ...
        fun _individuals(): Optional<JsonField<List<IndividualInDirectory>>> = Optional.ofNullable(individuals)
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<IndividualInDirectory>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<HrisDirectoryListIndividualsPageAsync>>.forEach(action: (IndividualInDirectory) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
                        it.individuals().all(action)
                    }
                    .map {
