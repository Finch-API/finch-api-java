...
class HrisDirectoryListPageAsync private constructor(
    ...
    fun getNextPageParams(): Optional<HrisDirectoryListParams> {
      ...
      return Optional.of(HrisDirectoryListParams.builder().from(params).offset(paging().flatMap { it.offset()}.orElse(0) + individuals().count()).build());
    }

    fun getNextPage(): CompletableFuture<Optional<HrisDirectoryListPageAsync>> {
      return getNextPageParams().map {
        directoryService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<IndividualInDirectory>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<HrisDirectoryListPageAsync>>.forEach(action: (IndividualInDirectory) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
